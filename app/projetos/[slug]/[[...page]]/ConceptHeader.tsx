...
export default function ConceptHeader({project,active}:{project:DemoProject;active:string}) {
 ...
 return <header className="concept-header">
  <a className="concept-brand" href={href('inicio')}><img src={`/brand/${project.slug}.png`} alt={`Logo ${project.title}`} width={58} height={58}/><span>{project.title}<small>{project.category}</small></span></a>
