...
function ProjectHeader({ project, active }: { project: DemoProject; active: string }) {
  return <header className="showcase-header"><a className="showcase-brand" href={demoHref(project.slug, 'inicio')}><b>{project.mark}</b><span>{project.title}<small>{project.category}</small></span></a><nav aria-label={`Navegação de ${project.title}`}>{project.navigation.map((item) => <a key={item.slug} className={active === item.slug ? 'active' : ''} href={demoHref(project.slug, item.slug)}>{item.label}</a>)}</nav><a className="showcase-nav-cta" href={demoHref(project.slug, project.navigation[3].slug)}>{project.navigation[3].label} <ArrowUpRight size={15} /></a></header>;
}

function ProjectFooter({ project, profile }: { project: DemoProject; profile: Profile }) {
  return <footer className="showcase-footer"><div><b>{project.mark}</b><h3>{project.title}</h3><p>{project.description}</p></div><div><span>Explore</span>{project.navigation.map((item) => <a key={item.slug} href={demoHref(project.slug, item.slug)}>{item.label}</a>)}</div><div><span>Atendimento</span><p>{profile.address}</p><p>{profile.hours}</p></div><div className="showcase-footer-bottom"><span>Projeto apresentado por Zoryon Web</span><span>Lavras · MG · 2026</span></div></footer>;
}
...
function HomePage({ project, profile }: { project: DemoProject; profile: Profile }) {
  return <><section className="showcase-hero"><img src={profile.hero} alt={`Imagem principal de ${project.title}`} /><div className="showcase-hero-shade" /><div className="showcase-hero-copy"><span>{project.heroKicker}</span><h1>{project.tagline}</h1><p>{project.description}</p><div><a className="showcase-primary" href={demoHref(project.slug, project.navigation[1].slug)}>{project.primaryAction} <ArrowRight /></a><a className="showcase-text-link" href={demoHref(project.slug, project.navigation[2].slug)}>Conhecer a marca</a></div></div><div className="showcase-hero-note"><strong>01</strong><span>{profile.promise}</span></div></section><section className="showcase-trust"><div><Clock3 /><span>{profile.hours}</span></div><div><MapPin /><span>{profile.address}</span></div><div><ShieldCheck /><span>Atendimento direto e seguro</span></div></section><section className="showcase-selection"><div className="showcase-section-title"><span>Seleção em destaque</span><h2>{project.navigation[1].label}<br />para escolher com calma.</h2><a href={demoHref(project.slug, project.navigation[1].slug)}>Ver tudo <ArrowRight /></a></div><div className="showcase-product-grid">{project.offers.map((offer, index) => <article key={offer.title}>{offer.image ? <img src={offer.image} alt={offer.title} /> : <div className="showcase-card-image" style={{ backgroundImage: `url(${profile.hero})`, backgroundPosition: `${20 + index * 22}% center` }} />}<div><small>0{index + 1}</small><h3>{offer.title}</h3><p>{offer.description}</p><strong>{offer.price}</strong></div></article>)}</div></section><section className="showcase-story-block"><div className="showcase-story-image"><img src={profile.hero} alt={`Universo visual de ${project.title}`} /><span>{project.visualWord}</span></div><div><span>Nossa essência</span><h2>{project.storyTitle}</h2><p>{project.story}</p><ul>{project.highlights.map((item) => <li key={item}><Check /> {item}</li>)}</ul><a href={demoHref(project.slug, project.navigation[2].slug)}>Conhecer em detalhes <ArrowUpRight /></a></div></section><section className="showcase-numbers">{project.stats.map((stat) => <div key={stat.label}><strong>{stat.value}</strong><span>{stat.label}</span></div>)}</section><ClosingCta project={project} /></>;
}
