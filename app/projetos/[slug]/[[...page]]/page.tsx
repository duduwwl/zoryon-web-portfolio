...
function ProjectHeader({ project, active }: { project: DemoProject; active: string }) {
  if (project.slug === 'serra-alta-imoveis') return <ConceptHeader project={project} active={active}/>;
...
function ProjectFooter({ project, profile }: { project: DemoProject; profile: Profile }) {
  if (project.slug === 'serra-alta-imoveis') {
    return <footer className="concept-footer"><div className="concept-footer-intro"><a className="concept-brand" href={demoHref(project.slug,'inicio')}><img src={`/brand/${project.slug}.png`} alt="" width={58} height={58}/><span>{project.title}<small>Seu lugar em Lavras</small></span></a><p>Da primeira busca à próxima mudança, espaço para escolher com calma. Conheça possibilidades de morar e investir em Lavras.</p></div><nav aria-label={`Links do rodapé de ${project.title}`}><h2>Encontre seu lugar</h2>{project.navigation.map(item=><a key={item.slug} href={demoHref(project.slug,item.slug)}>{item.label}<ArrowUpRight size={14}/></a>)}</nav><div className="concept-footer-info"><h2>Vamos conversar</h2><p><MapPin size={18}/><span>Lavras, Minas Gerais<br/><small>Visitas com hora marcada</small></span></p><p><Clock3 size={18}/><span>Segunda a sexta<br/><small>Das 9h às 18h</small></span></p></div><div className="concept-footer-bottom"><span>Conceito demonstrativo da Zoryon Web · não é um estabelecimento real</span><a href="/#projetos">Voltar ao portfólio <ArrowUpRight size={14}/></a></div></footer>;
  }
